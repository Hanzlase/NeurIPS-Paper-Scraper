...
class NeurIPSScraper(tk.Tk):
        ...
        try:
            async with session.get(pdf_url, headers=headers) as response:
                if response.status == 200:
                    ...
                    return True
                self.log(f"Failed to download {
                         pdf_url}: HTTP {response.status}")
                return False
    ...
    async def scrape_year(self, session: aiohttp.ClientSession, year: int) -> List[Dict]:
        # Use the old domain for earlier years, and the new domain for 2019 and later
        if year < 2019:
        ...
        try:
            async with session.get(base_url, headers=headers) as response:
                if response.status != 200:
                    self.log(f"Failed to fetch year {
                             year}: HTTP {response.status}")
                    return papers
                ...
                for paper_link in paper_links:
                    try:
                        ...
                        if 'Abstract' in abstract_url:
                            paper_hash = abstract_url.split(
                                '/')[-1].replace('-Abstract.html', '')
    ...
    async def scrape_metadata_async(self):
        ...
        # Creating an SSL context that disables certificate verification
        ...
        async with aiohttp.ClientSession(timeout=timeout, connector=connector) as session:
            ...
            with open(csv_path, 'w', newline='', encoding='utf-8') as f:
                writer = csv.DictWriter(
                    f, fieldnames=['title', 'authors', 'year', 'pdf_link'])
                writer.writeheader()
    ...
    async def download_pdfs_async(self):
        ...
        # Reuse the same SSL context to disable certificate verification
        ssl_context = ssl.create_default_context()
        ssl_context.check_hostname = False
        ssl_context.verify_mode = ssl.CERT_NONE
        connector = aiohttp.TCPConnector(ssl=ssl_context)
    ...
    # Using Threaded Callbacks to resolve lagging issue
    def scrape_metadata(self):
        ...
        def run_scrape():
            ...
            # Use self.after to update the UI on the main thread
